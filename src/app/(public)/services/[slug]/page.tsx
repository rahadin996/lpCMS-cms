...
import {
  FiArrowRight, FiArrowUp, FiUsers, FiMapPin, FiTrendingUp,
  FiBarChart2, FiCompass, FiCpu, FiGlobe, FiLayers, FiAward,
  FiTwitter, FiLinkedin, FiInstagram, FiFacebook, FiStar, FiChevronDown
} from 'react-icons/fi'
...
const iconMap: Record<string, IconType> = {
  ...
  FiFacebook, FiStar, FiChevronDown
}
...
const FAQItem = ({
  ...
  return (
    ...
    >
      ...
      >
        {faq.q}
        {open ? <FiChevronDown /> : <FiChevronDown className="rotate-180" />}
      </button>
      <AnimatePresence>
        {open && (
          <motion.div
...
export default function ServiceDetailPage() {
    ...
    if (content?.testimonials?.length) {
      const interval = setInterval(() => {
        setActiveTestimonial(
          prev => (prev + 1) % (content.testimonials?.length ?? 1)
        )
      }, 5000)
  ...
  if (loading) {
    return <div className="text-center py-20">Memuat detail layanan...</div>
  }

  if (!content) {
    return (
      <div className="text-center py-20">
        <p className="text-gray-500">Konten tidak ditemukan. Silakan edit di admin panel.</p>
  ...
  // ========== ANIMATION VARIANTS (DIPERBAIKI DENGAN as const) ==========
  const fadeUp = {
    ...
    <div ref={containerRef} className="bg-white overflow-x-hidden relative min-h-screen">
        ...
        <div className="relative z-10 text-center max-w-6xl mx-auto px-4 text-white">
          ...
          >
            <Link
              ...
              className="group inline-flex items-center gap-2 bg-white text-gray-900 px-8 py-3 rounded-full font-medium shadow-lg hover:shadow-xl transition"
            >
              {hero.button_text || 'Konsultasi Sekarang'}{' '}
              <FiArrowRight className="group-hover:translate-x-1 transition" />
            </Link>
            <Link
              href={hero.secondary_button_link || '/projects'}
              className="inline-flex items-center gap-2 border border-white/30 backdrop-blur-sm px-8 py-3 rounded-full hover:bg-white/10 transition"
            >
        ...
        <section className="py-20 px-4 max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {stats.map((stat, idx) => {
              ...
              const color =
                stat.color ||
                ['from-blue-500 to-cyan-500', 'from-emerald-500 to-teal-500', 'from-amber-500 to-orange-500', 'from-purple-500 to-pink-500'][
                  idx % 4
                ]
              return (
                <StatCounter
        ...
        <section className="py-20 px-4 max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              {about.description2 && (
                <p className="mt-4 text-gray-600 leading-relaxed">{about.description2}</p>
              )}
            </motion.div>
            ...
            >
              {(about.features || []).map((feat, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="bg-gray-50/80 p-5 rounded-xl text-center hover:shadow-lg transition"
                  >
              ...
              {benefits.map((b, i) => {
                ...
                return (
                  <motion.div
                    key={i}
                    variants={fadeUp}
                    className="flex items-center gap-4 bg-white p-5 rounded-xl shadow-md hover:shadow-lg transition group"
                  >
        ...
        <section className="py-20 px-4 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {timeline.map((t, i) => (
              ...
              >
                <div className="w-16 h-16 mx-auto rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-xl font-bold mb-3">
                  {i + 1}
                </div>
      ...
      {gallery.length > 0 && (
        <section className="py-20 px-4 bg-white">
          <div className="max-w-7xl mx-auto text-center">
            <span className="inline-block px-4 py-1.5 text-sm font-medium text-cyan-600 bg-cyan-100 rounded-full mb-4">
              GALERI
            </span>
                ...
                {testimonials.map((_, idx) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition ${
                      activeTestimonial === idx ? 'bg-blue-600 w-4' : 'bg-gray-300'
                    }`}
                  />
                ))}
              </div>
      ...
      <section className="relative py-28 px-4 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto text-center text-white">
          ...
          <p className="mt-4 text-lg text-white/80">
            Tim ahli kami siap membantu mewujudkan rencana Anda.
          </p>
          <div className="mt-10">
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 bg-white text-gray-900 px-10 py-4 rounded-full font-semibold shadow-xl hover:shadow-2xl transition"
            >
